...
func consistentBaseline(work []authorityv1.WorkItem, leases []authorityv1.CapabilityLease, pending []pendingBaselineSaga) bool {
	...
	for _, lease := range leases {
		...
		if !found || covered[lease.BeadID] || !lease.Active || item.NativeStatus != "in_progress" || item.LifecycleState != authorityv1.LifecycleInProgress || item.ClaimAttemptID != lease.CanonicalClaimAttemptID || item.Version != lease.ClaimVersion || !equalStrings(item.ExclusivePaths, lease.ExclusivePaths) || !equalLabels(item.Labels, lease.Labels) {
			return false
		}
	...
	for _, saga := range pending {
		...
		case gateway.ClaimPhaseCanonical:
			if item.LifecycleState != authorityv1.LifecycleInProgress || saga.Work.ClaimAttemptID == "" || item.ClaimAttemptID != saga.Work.ClaimAttemptID || !sameStoredWork(item, saga.Work) {
				return false
			}
		default:
			return false
