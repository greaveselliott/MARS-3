...
// EnterEffect holds the exact lease row through one W-001 synthetic effect.
// It deliberately does not lock the project row: journal append and revoke use
// project-then-lease ordering, so the effect guard must never invert it.
func (store *Store) EnterEffect(ctx context.Context, fence authorityv1.FencingTuple) (authorityv1.CapabilityLease, func(), error) {
	tx, err := store.beginScoped(ctx, fence.TenantID, fence.ProjectID, pgx.ReadCommitted)
	if err != nil {
		return authorityv1.CapabilityLease{}, nil, err
	}
	release := func() { _ = tx.Rollback(context.Background()) }
	if _, err := store.verifyProject(ctx, tx, fence.TenantID, fence.ProjectID, false); err != nil {
		release()
		return authorityv1.CapabilityLease{}, nil, err
	}
	lease, err := loadLease(ctx, tx, fence.TenantID, fence.ProjectID, fence.LeaseID, true)
	if errors.Is(err, pgx.ErrNoRows) {
		release()
		return authorityv1.CapabilityLease{}, nil, ErrLeaseNotFound
	}
	if err != nil || !lease.Active || !lease.ExpiresAt.After(store.now().UTC()) || !leaseMatchesFence(lease, fence) {
		release()
		return authorityv1.CapabilityLease{}, nil, ErrLeaseFence
	}
	var released bool
	return lease, func() {
		if !released {
