...
func TestExecuteSyntheticEffectSerializesRevocationAtEffectBoundary(t *testing.T) {
	...
	request := authorityv1.EffectValidationRequest{
		Fence: fenceFromLease(lease), EffectID: "effect-linearized", Path: "internal/authority/gateway/effect.go", TraceRef: "trace-effect-linearized",
	}
	entered, releaseEffect := make(chan struct{}), make(chan struct{})
	result := make(chan error, 1)
	var effects atomic.Int32
	go func() {
		_, err := service.ExecuteSyntheticEffect(context.Background(), principal, request, func(context.Context) error {
			effects.Add(1)
			close(entered)
			<-releaseEffect
			return nil
	...
	case <-time.After(time.Second):
		t.Fatal("synthetic effect did not reach its fenced boundary")
	}
	security := reader()
	...
	go func() {
		_, err := service.RevokeLease(context.Background(), security, authorityv1.RevokeLeaseRequest{
			TenantID: lease.TenantID, ProjectID: lease.ProjectID, LeaseID: lease.LeaseID,
			FenceGeneration: lease.FenceGeneration, LeaseEpoch: lease.LeaseEpoch, Reason: "security-test-revoke", TraceRef: "trace-effect-revoke",
		})
		revoked <- err
	}()
	select {
	case err := <-revoked:
		t.Fatalf("revocation crossed an in-flight effect guard: %v", err)
	case <-time.After(25 * time.Millisecond):
	}
	close(releaseEffect)
	if err := <-result; err != nil {
		t.Fatalf("linearized synthetic effect: %v", err)
	}
	if err := <-revoked; err != nil {
		t.Fatalf("revoke after synthetic effect: %v", err)
	}
	if effects.Load() != 1 {
		t.Fatalf("effects=%d, want one", effects.Load())
	}
	if _, err := service.ExecuteSyntheticEffect(context.Background(), principal, request, func(context.Context) error {
		effects.Add(1)
		return nil
	}); err == nil {
		t.Fatal("revoked lease authorized a later synthetic effect")
	}
	stored, err := sagas.GetLease(context.Background(), lease.TenantID, lease.ProjectID, lease.LeaseID)
	if err != nil || stored.State != authorityv1.LeaseRevoked || effects.Load() != 1 {
		t.Fatalf("stored=%#v err=%v effects=%d", stored, err, effects.Load())
	}
}

func TestExecuteSyntheticEffectRecordsUnknownOutcomeAndRejectsNilEffect(t *testing.T) {
	clock := time.Date(2026, 8, 27, 2, 0, 0, 0, time.UTC)
	service, _, _, events, principal, lease := lifecycleFixture(t, &clock)
	principal.Capabilities = append(principal.Capabilities, authorityv1.CapabilityEffectValidate)
	request := authorityv1.EffectValidationRequest{
		Fence: fenceFromLease(lease), EffectID: "effect-unknown", Path: "internal/authority/gateway/effect.go", TraceRef: "trace-effect-unknown",
	}
	_, err := service.ExecuteSyntheticEffect(context.Background(), principal, request, func(context.Context) error {
		return errors.New("synthetic effect outcome is unknown")
	})
	assertDenial(t, err, authorityv1.ErrorUnknownEffect, ruleEffectUnknown, requiredStableClaim, allowedEffectRecheck)
	if len(events.events) != 6 || events.events[len(events.events)-1].Outcome != outcomeDenied {
		t.Fatalf("events=%#v, want claim triple plus effect intent, policy, and bounded denial", events.events)
	}
	_, err = service.ExecuteSyntheticEffect(context.Background(), principal, request, nil)
	assertDenial(t, err, authorityv1.ErrorInvalidRequest, ruleEffectInvalid, requiredEffectPath, allowedEffectRecheck)
}
...
func effectFixture(now time.Time) (authorityv1.WorkItem, authorityv1.CapabilityLease, authorityv1.EffectValidationRequest) {
	...
	lease := authorityv1.CapabilityLease{
		...
		AttemptID: "delivery-attempt-001", CanonicalClaimAttemptID: work.ClaimAttemptID, IdempotencyKey: "test-key", FenceGeneration: "generation-lease-001", LeaseEpoch: 7,
